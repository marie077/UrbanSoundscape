...
Serial feather;
String portName = "/dev/cu.urbanSoundscape";
...
void draw() {
  if (feather.available() > 0) {
    //println(feather.read());
    pluck(int(feather.read()));
  }
  ...
  if (!locked && millis() - lastFlipTime >= flipBackTime) {
  ...
  if (locked && millis() - lastFlipTime >= flipBackTime) {
    ...
    locked = false;    // Unlock the visuals
...
void pluck(int i) {
  ...
  locked = false; // Unlock the visuals if the duration has passed
